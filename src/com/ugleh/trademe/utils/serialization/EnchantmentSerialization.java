...
/**
 * A class to help with the serialization of Enchantments. Because of the ability to add unsafe
 * enchantments (item and enchantment level are ignored), enchantments are serialized in a different
 * way than like ChestShop. ChestShop takes the enchantment id, converts it into a String, and
 * appends the level. It then iterates this process for each enchantment. Afterwards, it has a very
 * long number, which it then conveters to a base-32 String. <br/> <br/> This class serializes it
 * much differently, and it is in fact more human readable. The process can be explained as such:
 *
 * <pre>
...
public class EnchantmentSerialization {
        ...
        for (int i = 0; i < enchants.length; i++) {
            ...
            if (ench.length < 2) {
                throw new IllegalArgumentException(
                        serializedEnchants + " - Enchantment " + i + " (" + enchants[i]
     ...
     * @return A map of enchantments and their levels using the old (ChestShop compatible)
     * enchantment code.
     */
    @SuppressWarnings("deprecation")
